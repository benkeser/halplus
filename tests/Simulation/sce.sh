...
#### Dave's note ####
# helper fn for getting rid of carriage returns
# tr -d '\r' <cent.R> centMod.R
####
...
ANALYSIS='nosl4'    # change for every analysis you run (2nd arg)
...
SCRIPT='./centMod.R'       # your code as (R or Python) script (1st arg)
...
if [[ -z $RETRYFAILED ]]; then 
    ...
    for i in $(seq 1 $numjobs); do
        ...
        sbatch --dependency=afterok:${waitforjobs} --job-name=${ANALYSIS} \
               ...
               --wrap="${SCRIPT} run ${arrid} $3 $4 $5 $6 $7 $8 $9" --requeue --exclusive
